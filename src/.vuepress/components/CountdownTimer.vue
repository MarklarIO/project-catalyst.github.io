  ...
  <v-sheet elevation="1" rounded :class="expired ? 'expired':''">
    ...
    <div class="counter">
      <div>
        <div>
          <div>days</div>
          <div :class="displayPrefix(daysLeft)">
            {{daysLeft}}
          </div>
        </div>
        <div>:</div>
      ...
      <div>
        <div>
          <div>hours</div>
          <div :class="displayPrefix(hoursLeft)">
            {{hoursLeft}}
          </div>
        </div>
        <div>:</div>
      ...
      <div>
        <div>
          <div>minutes</div>
          <div :class="displayPrefix(minutesLeft)">
            {{minutesLeft}}
          </div>
        </div>
        <div>:</div>
      ...
      <div>
        <div>
          <div>seconds</div>
          <div :class="displayPrefix(secondsLeft)">
            {{secondsLeft}}
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    hoursLeft: function () {
      ...
      var modTime = this.countdownDT
        .minusDays(this.daysLeft)
      return this.currentDT.until(modTime, ChronoUnit.HOURS)
    },
    minutesLeft: function () {
      ...
      var modTime = this.countdownDT
        .minusDays(this.daysLeft)
        .minusHours(this.hoursLeft)
      return this.currentDT.until(modTime, ChronoUnit.MINUTES)
    },
    secondsLeft: function () {
      ...
      var modTime = this.countdownDT
        .minusDays(this.daysLeft)
        .minusHours(this.hoursLeft)
        .minusMinutes(this.minutesLeft)
      return this.currentDT.until(modTime, ChronoUnit.SECONDS)
    }
  },
  methods: {
    inputGuard: function () {
      return (
        (this.countdownDT != null)
        && (this.currentDT   != null)
        && (!this.currentDT.isAfter(this.countdownDT))
      );
    },
  ...
  // Conforming to VuePress Browser API Restrictions
